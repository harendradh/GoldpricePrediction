"""Documentation Quality skill · docstrings · README · API docs.

Purpose
-------
Catches the documentation gaps that make code unreadable six months later:
public functions added without docstrings, TODO/FIXME without owner,
README sections gone stale.

Methodology
-----------
1. **Deterministic regex pass** — `DOC_PATTERNS` (PEP-257 + CWE-546).
2. **LLM contextual pass** — surfaces docstring-says-X-but-code-does-Y
   mismatches and missing edge-case docs.

Authoritative standards consulted
---------------------------------
- PEP-257 (Docstring Conventions)
- Google Python Style Guide · Docstrings section
- CWE-546 (Suspicious Comment)
"""
from __future__ import annotations

from Agents.Core.model import DatabricksClaude, ModelError
from Agents.Core.observability import get_logger
from Agents.Core.schemas import Dimension, SkillContext, SkillResult
from skills._shared.llm_helper import apply_memory_adjustments, call_with_findings, make_finding
from skills._shared.patterns import DOC_PATTERNS
from skills._shared.standards import (
    CLEAN_CODE,
    CWE_546_SUSPICIOUS_CMT,
    GOOGLE_DOC_STYLE,
    PEP_257_DOCSTRINGS,
)
from skills.base import Skill, register_skill

logger = get_logger(__name__)


@register_skill
class DocumentationQualitySkill(Skill):
    name = "documentation_quality"
    description = "Docstring coverage · README freshness · TODO ownership · PEP-257 grounded"
    dimensions = [Dimension.DOCUMENTATION.value]

    STANDARDS = [PEP_257_DOCSTRINGS, GOOGLE_DOC_STYLE, CWE_546_SUSPICIOUS_CMT, CLEAN_CODE]

    async def run(self, ctx: SkillContext, model: DatabricksClaude | None = None) -> SkillResult:
        det = self._deterministic(ctx)
        llm: list = []
        telemetry = {"model_calls": 0, "input_tokens": 0, "output_tokens": 0, "estimated_cost_usd": 0.0}
        fallback = True
        try:
            sys_p = (
                "You audit documentation hygiene. Your beat: public functions "
                "without docstrings, TODO without owner / ticket, README "
                "sections that have gone stale relative to the code, "
                "docstring-says-X-but-code-does-Y mismatches. Cite PEP-257 "
                "or Google Doc Style in every finding."
            )
            llm, telemetry = await call_with_findings(
                model=model, system_prompt=sys_p,
                user_prompt=f"Documentation review.\n\nDiff:\n```\n{self.diff_blob(ctx, 4500)}\n```",
                skill_name=self.name, default_dimension=Dimension.DOCUMENTATION,
                standards=self.STANDARDS,
            )
            fallback = False
        except ModelError as exc:
            logger.warning("skill.docs.fallback", error=str(exc)[:200])
        merged = apply_memory_adjustments(self._dedupe(det + llm), ctx)
        for f in merged:
            f.auto_postable = f.confidence >= 72
        return SkillResult(findings=merged, fallback_used=fallback, **telemetry,
                           payload={"deterministic": len(det), "llm": len(llm)})

    def _deterministic(self, ctx: SkillContext) -> list:
        if ctx.pr is None:
            return []
        out = []
        for fc in ctx.pr.files:
            for hunk in fc.hunks:
                start = hunk.get("new_start", 1)
                text = "\n".join(hunk.get("lines", []))
                for spec in DOC_PATTERNS:
                    if spec.regex.search(text):
                        loc, quote = start, text[:120]
                        for off, ln in enumerate(hunk.get("lines", [])):
                            if spec.regex.search(ln):
                                loc, quote = start + off, ln.lstrip("+").strip()
                                break
                        out.append(make_finding(
                            rule_id=spec.rule_id, skill=self.name,
                            dimension=spec.dimension, severity=spec.severity,
                            confidence=78, file=fc.path, line=loc,
                            title=spec.title, why=spec.why, fix=spec.fix,
                            quote=quote[:200], references=spec.references,
                        ))
        return out

    @staticmethod
    def _dedupe(findings):
        seen = set(); out = []
        for f in findings:
            k = (f.rule_id, f.file, f.line_start)
            if k not in seen:
                seen.add(k); out.append(f)
        return out
